...
Una de las tareas centrales consiste en identificar si distintos programas de diferentes licenciaturas cubren contenido equivalente o sustancialmente solapado, y si existen duplicaciones implícitas dentro de un mismo plan de estudios---información de apoyo para las decisiones de revisión curricular que corresponden a las instancias académicas competentes.

\begin{recuadro}{¿Cómo se mide la similitud entre programas?}
Cada programa de UEA se convierte en un vector numérico: a cada término técnico se le asigna un peso que crece con su frecuencia en ese programa y decrece con la fracción del corpus en que aparece---los términos comunes a casi todos los planes aportan poca información discriminativa; los propios de una disciplina, mucha.
La similitud entre dos programas es el coseno del ángulo entre sus vectores, un número $s \in [0,1]$: vale $s = 1$ si ambos programas comparten exactamente el mismo perfil de términos relevantes, y $s = 0$ si no tienen ningún término significativo en común.
...
\begin{tabular}{cl}
\toprule
\textbf{Rango de }$s$ & \textbf{Interpretación operacional} \\
\midrule
$[0.85,\;1.00]$ & Equivalencia muy alta o textual \\
$[0.65,\;0.85)$ & Contenidos ampliamente solapados \\
$[0.45,\;0.65)$ & Traslape de contenidos significativo \\
$[0.30,\;0.45)$ & Vocabulario técnico parcialmente compartido \\
$[0.00,\;0.30)$ & Sin traslape significativo \\
\bottomrule
\end{tabular}
\end{center}
\smallskip

Esta medida léxica subestima la similitud cuando dos programas cubren el mismo contenido con vocabulario distinto: ``Liderazgo organizacional'' y ``Gestión de equipos de trabajo'' obtienen $s$ baja aunque sean equivalentes.
Para corregir este sesgo, un modelo de lenguaje grande re-evalúa directamente los pares candidatos y emite una puntuación semántica revisada que reemplaza la puntuación léxica.
\end{recuadro}
...
En la primera, los campos \textit{Objetivo General} y \textit{Contenido Sintético} de cada programa se representan como vectores TF-IDF (frecuencia de término--frecuencia inversa de documento) en un espacio de 8\,000 dimensiones, y se calcula la similitud coseno entre cada par, produciendo una matriz inicial de $626 \times 626$ entradas.
En la segunda, todos los pares con $s_{\mathrm{TF\text{-}IDF}} \geq 0.20$ (1\,505 pares candidatos) son evaluados por el modelo de lenguaje grande (LLM) \textit{claude-haiku-4-5}, que devuelve una puntuación semántica revisada $s_{\mathrm{LLM}} \in [0,1]$ y una justificación en español.
El resultado es la \textit{matriz híbrida}: más precisa que el análisis TF-IDF solo porque corrige falsos positivos (vocabulario genérico compartido sin equivalencia real de contenidos) y falsos negativos (programas idénticos con redacciones léxicamente distintas).

El análisis produce tres clases de medida.
La primera es la \textit{similitud par a par}: la matriz híbrida asigna a cada uno de los $\binom{626}{2} = 195\,375$ pares posibles de UEAs un puntaje $s \in [0,1]$, donde $s = 1$ indica equivalencia textual y $s < 0.30$ indica ausencia de traslape significativo.
La segunda es el \textit{recuento de pares equivalentes} entre licenciaturas a dos umbrales operacionales: $s \geq 0.30$ para pares inter-plan y $s \geq 0.40$ para pares intra-plan---umbral más estricto en el segundo caso, dado que la afinidad disciplinar entre UEAs de un mismo plan es estructuralmente mayor que entre licenciaturas distintas.
La tercera es la \textit{similitud media-máxima} $\bar{s}(A,B)$ entre planes completos: para cada UEA del plan $A$ se identifica la UEA más similar existente en $B$, y se promedia esa similitud máxima sobre todas las UEAs de $A$; el indicador es asimétrico por diseño---$\bar{s}(A,B)$ puede diferir de $\bar{s}(B,A)$ cuando los planes difieren en tamaño o diversidad---y cuantifica el grado de cobertura que $B$ ofrece para los contenidos de $A$.

Los hallazgos revelan un traslape más extenso y estructuralmente más complejo que el detectable mediante análisis léxico.
El par Industrial--Mecánica concentra la mayor densidad del corpus: 38~UEAs equivalentes a $s \geq 0.70$---seis veces más que el segundo par de mayor traslape---en áreas de manufactura, materiales, termodinámica, métodos numéricos y UEAs de formación amplia; 38 UEAs equivalentes representan el 49\,\% de las UEAs no-TG de Industrial y el 35\,\% de las de Mecánica.
La Licenciatura en Física actúa como nodo transversal: comparte UEAs con las diez licenciaturas y acumula 100~pares inter-plan a $s \geq 0.30$, la mayor conectividad del corpus.
El bloque Ambiental--Química aporta la segunda densidad más alta (14~UEAs a $s \geq 0.70$), con contenidos de ingeniería ambiental compartidos en una medida que supera lo atribuible a vocabulario disciplinar común.
Cuatro familias de contenido cruzan entre cinco y nueve planes con alta similitud sin coordinación formal entre licenciaturas: Ecuaciones Diferenciales (9 planes, $s = 1.00$), Liderazgo y Emprendimiento (9 planes, detectable solo mediante el LLM porque TF-IDF le asignaba $s \approx 0.21$--$0.25$), Educación Ambiental y Cambio Climático (6~o más planes, $s \geq 0.95$), y un núcleo matemático de cuatro UEAs presentes en 5~planes cada una con $s \geq 0.98$.
En el análisis intra-plan se identificaron tres duplicados documentales---en Química, Física e Industrial---y el plan de Civil registra la mayor densidad del corpus con 61~pares a $s \geq 0.40$, resultado de objetivos convergentes redactados independientemente por distintos departamentos.
El enriquecimiento semántico con LLM fue determinante: incrementó los pares inter-plan detectados en 154\,\% y los intra-plan en 75\,\%, identificando 455~pares con equivalencia semántica real que el análisis léxico habría descartado.

Todos los resultados de este reporte---tablas, mapas de calor, grafo y proyección t-SNE (\textit{t-distributed Stochastic Neighbor Embedding})---provienen de la matriz híbrida.
...
Primera: viola la ley de Zipf; en cualquier corpus natural, unos pocos términos son extremadamente frecuentes y poco discriminativos (``ecuaciones'', ``análisis'', ``sistemas''), mientras que los términos técnicamente específicos son raros, de modo que un vector de frecuencias brutas queda dominado por términos genéricos.
Segunda: los documentos más largos producen coordenadas sistemáticamente mayores, sesgando la comparación.
...
El factor $\log(N/n_t)$ es la \textit{frecuencia inversa de documento} (IDF), introducida por \citet{SparkJones1972}.
...
\citet{Robertson2004} proporciona la justificación bayesiana formal de esta intuición: el IDF puede derivarse como logaritmo de la razón de verosimilitudes de encontrar el término en un documento relevante frente a uno irrelevante.
...
La preferencia por TF-IDF sobre las representaciones neurales densas (BERT, modelos de lenguaje) en esta primera fase obedece a tres razones.
\textit{Interpretabilidad}: cada coordenada de un vector TF-IDF corresponde a un término específico, de modo que la similitud entre dos programas resulta explicable en términos del vocabulario técnico que comparten.
...
La distancia euclidiana $\|\mathbf{v}_{d_i} - \mathbf{v}_{d_j}\|$ depende de la magnitud de los vectores: un programa de 500 palabras tendrá coordenadas mayores que uno de 150 palabras sobre el mismo tema, y su distancia euclidiana a cualquier otro programa será sistemáticamente mayor---un sesgo indeseable.
...
Las UEAs del Tronco General comparten programa por definición; incluirlas en el análisis produciría pares trivialmente idénticos que contaminarían las métricas de traslape entre licenciaturas.
...
Para visualizarla como mapa de calor es necesario reordenar filas y columnas de modo que las UEAs de contenido similar queden adyacentes; de lo contrario, el mapa se reduce a ruido visual.
Este reordenamiento se obtiene mediante el \textit{agrupamiento jerárquico aglomerativo de Ward} \citep{Ward1963}.

El método parte de la configuración en que cada UEA forma su propio grupo y fusiona iterativamente los dos grupos cuya unión incrementa mínimamente la varianza intra-cluster total.
...
La ventaja de Ward frente a otros criterios de enlace (completo, promedio, simple) es que estos últimos presentan el \textit{efecto de encadenamiento}: un único par de UEAs con alta similitud puede conectar artificialmente dos grupos temáticamente distintos.
Ward penaliza las fusiones que incrementan la heterogeneidad interna, produciendo un dendrograma cuya estructura refleja con mayor fidelidad la organización temática del corpus.
...
El análisis de componentes principales (PCA) produciría una proyección lineal que captura la varianza global pero no la estructura de grupos locales: en espacios de alta dimensión, los datos residen frecuentemente en variedades no lineales que PCA no puede desplegar.
...
En la proyección bidimensional, define las probabilidades análogas $q_{ij}$ mediante una distribución de Student con un grado de libertad---de cola más pesada que la gaussiana.
El algoritmo minimiza la divergencia de Kullback--Leibler $\mathrm{KL}(P\,\|\,Q)$ entre ambas distribuciones, forzando que los vecinos en el espacio original lo sean también en el plano.

La cola pesada de la distribución de Student en la proyección resuelve el \textit{problema de aglomeración} (\textit{crowding problem}): en alta dimensión, numerosos puntos pueden situarse a distancia moderada de un punto central; en dos dimensiones no hay espacio suficiente para todos ellos si se emplean distribuciones de colas ligeras, lo que produce mapas comprimidos e ilegibles \citep{Maaten2008}.
...
Siguiendo la recomendación de \citet{Maaten2008}, la proyección se aplica en dos etapas: primero se reduce el espacio a 50 componentes principales con PCA---eliminando dimensiones de ruido y acelerando el cálculo---y sobre ese espacio de 50 dimensiones se ejecuta t-SNE.
...
La elección del operador \textit{max} frente a un promedio global responde a la naturaleza del problema: un promedio global quedaría dominado por las numerosas UEAs sin traslape, diluyendo las señales reales de coincidencia.
...
La asimetría es informativamente valiosa: un valor alto de $\simAB$ con $\bar{s}(B,A)$ bajo indica que el plan $A$ está cubierto por $B$ pero no a la inversa---una relación de inclusión unilateral de contenidos.
...
La similitud coseno captura el traslape léxico pero es insensible a equivalencias semánticas expresadas con vocabulario distinto.
...
El procedimiento comprende tres pasos.
Primero, se identifican como \textit{candidatos} todos los pares con $s_{\mathrm{TF\text{-}IDF}} \geq 0.20$---umbral bajo que captura prácticamente todo par con algún vocabulario compartido.
Segundo, para cada par candidato se envían al LLM los textos completos del \textit{Objetivo General} y el \textit{Contenido Sintético} de ambas UEAs, junto con una rúbrica de cinco niveles (idéntico, muy alto, alto, moderado, bajo); el modelo devuelve una puntuación $s_{\mathrm{LLM}} \in [0,1]$ y una breve justificación en español.
...
La figura~\ref{fig:scatter_llm} muestra la relación entre ambas puntuaciones para los 1\,505 pares candidatos, ilustrando las dos correcciones que el LLM introduce respecto al análisis léxico.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.88\textwidth]{Figuras/scatter_tfidf_llm.png}
  \caption{Puntuación TF-IDF (eje $x$) frente a puntuación LLM (eje $y$) para los 1\,505 pares
    candidatos ($s_{\mathrm{TF\text{-}IDF}} \geq 0.20$).
    \emph{Cómo leer la figura.}
    Cada punto es un par de UEAs; su posición refleja cómo los dos métodos evalúan ese par.
    Los puntos \emph{por debajo de la diagonal} ($s_{\mathrm{LLM}} < s_{\mathrm{TF\text{-}IDF}}$)
    son falsos positivos corregidos: el LLM determinó que el vocabulario compartido no
    corresponde a equivalencia real de contenidos---corrección especialmente frecuente en pares
    de Computación y Eléctrica con vocabulario genérico de software o electromagnetismo.
    Los puntos \emph{por encima de la diagonal} ($s_{\mathrm{LLM}} > s_{\mathrm{TF\text{-}IDF}}$)
    son falsos negativos recuperados: contenidos equivalentes redactados con vocabulario distinto,
    invisibles al análisis léxico.
    La \emph{región sombreada} delimitada por $s_{\mathrm{TF\text{-}IDF}} < 0.40$ y
    $s_{\mathrm{LLM}} \geq 0.40$ contiene los 455 pares con equivalencia semántica
    que TF-IDF habría descartado del análisis.
    El \emph{color} codifica el nivel asignado por el LLM:
    gris claro = traslape bajo o moderado; gris oscuro = alto;
    rojo (\textcolor{uamred}{$\bullet$}) = idéntico ($s_{\mathrm{LLM}} \geq 0.90$).
    La concentración de puntos rojos en la esquina superior derecha
    ($s_{\mathrm{TF}} \geq 0.40$, $s_{\mathrm{LLM}} \approx 1$) corresponde a los duplicados
    documentales y a las familias de Ecuaciones Diferenciales, Cálculo Integral y
    Educación Ambiental y Cambio Climático.
    La nube de puntos grises en $s_{\mathrm{TF}} \approx 0.21$--$0.25$,
    $s_{\mathrm{LLM}} \geq 0.70$, dentro de la región sombreada, corresponde a las instancias
    de Liderazgo y Emprendimiento (hallazgo~H6): vocabulario genérico que TF-IDF penaliza,
    contenido que el LLM identifica como prácticamente idéntico en nueve planes.}
  \label{fig:scatter_llm}
\end{figure}
...
Física encabeza los pares inter-plan con 100, seguida de Química con 99---reflejo de que estas licenciaturas comparten un núcleo de matemáticas, ciencia de materiales y UEAs de formación amplia que TF-IDF infravaloraba.
Civil presenta el salto más notable en pares intra-plan (de 8 a 61): el LLM detectó que numerosas UEAs comparten objetivos en las áreas de estructuras, hidráulica y materiales que las diferencias de redacción ocultaban.
Computación, en cambio, reduce de 57 a 33 pares intra-plan: el LLM corrigió los falsos positivos generados por TF-IDF a partir de vocabulario genérico de desarrollo de software.
...
Esta sección analiza la similitud entre UEAs de \textit{distintas} licenciaturas mediante cuatro instrumentos complementarios, cada uno orientado a una pregunta diferente sobre el mismo corpus: ¿cuánto se parece un plan a otro en conjunto? ¿Cuántas UEAs comparte cada par? ¿Qué UEAs específicas son las más ubicuas? ¿Cuáles son los pares individuales de mayor similitud?
...
La figura~\ref{fig:conteo_interplan} y la tabla~\ref{tab:matriz_interplan} cubren simultáneamente los $\binom{10}{2} = 45$ pares posibles de licenciaturas, mostrando cuántas UEAs comparte cada par a dos umbrales distintos.
A diferencia del mapa de calor anterior---que agrega la información en una métrica escalar por par---aquí se contabiliza directamente el número de UEAs equivalentes, la magnitud más relevante para la revisión curricular.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{Figuras/heatmap_interplan_conteo.png}
...
La figura~\ref{fig:barras_interplan} presenta los mismos conteos en formato de barras horizontales, haciendo visible la distribución de cola larga: Industrial--Mecánica concentra casi el triple de UEAs equivalentes que cualquier otro par.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.78\textheight,keepaspectratio]{Figuras/barras_interplan_top.png}
  \caption{Número de UEAs equivalentes a $s \geq 0.70$ por par de licenciaturas,
    ordenado de mayor a menor (44 pares con al menos una UEA equivalente).
    \emph{Cómo leer la figura.}
    Cada barra corresponde a un par; su longitud es el conteo de UEAs equivalentes.
    El par Computación--Metalúrgica, con cero UEAs a $s \geq 0.70$, no aparece en el eje.
    \emph{Distribución de cola larga}: Industrial--Mecánica (38) casi triplica al segundo par
    más alto, Civil--Mecánica (16); la mayoría de los 44 pares concentra entre 1 y 9 UEAs
    equivalentes.
    La figura complementa la tabla~\ref{tab:matriz_interplan} y el doble panel de la
    figura~\ref{fig:conteo_interplan}: donde éstos muestran la distribución espacial de los
    traslapes en la cuadrícula de licenciaturas, las barras hacen visible el peso relativo de
    cada par en el corpus total, permitiendo identificar de un vistazo qué pares concentran
    la mayor proporción del traslape inter-plan.}
  \label{fig:barras_interplan}
\end{figure}
...
\caption{UEAs presentes en tres o más licenciaturas con $s \geq 0.70$ (matriz híbrida).
  ...
  no excluida como Tronco General (TG) por el algoritmo de exclusión basado en clave.
...
La figura~\ref{fig:matriz_transversal} convierte la tabla~\ref{tab:transversal} en un mapa de calor que permite comparar visualmente el alcance y la intensidad de cada familia de contenido transversal sobre el conjunto de los diez planes.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth,height=0.72\textheight,keepaspectratio]{Figuras/matriz_transversal.png}
  \caption{Presencia de las 62 UEAs transversales en los diez planes de licenciatura
    (UEAs con al menos cinco licenciaturas con par equivalente a $s \geq 0.70$).
    \emph{Cómo leer la figura.}
    Las \emph{filas} son las UEAs transversales, ordenadas de mayor a menor número de
    licenciaturas en que tienen par equivalente.
    Las \emph{columnas} son las diez licenciaturas.
    La celda $(u, L)$ indica la similitud máxima $s_{\max}$ de la UEA $u$ hacia la
    licenciatura $L$: blanco $=$ sin par equivalente a $s \geq 0.30$; rojo intenso $=$ par
    idéntico o casi idéntico ($s \approx 1$).
    \emph{Patrones de lectura}:
    las filas uniformemente rojas de las primeras posiciones corresponden a las familias
    universales---Ecuaciones Diferenciales y Liderazgo y Emprendimiento presentes en 9 planes;
    las filas con celdas blancas dispersas son UEAs transversales dentro de un bloque
    disciplinar específico.
    Las columnas más oscuras en conjunto revelan qué licenciaturas concentran mayor número
    de contenidos compartidos con el resto; una columna con muchas celdas blancas indica
    un perfil de contenidos más diferenciado.
    La figura extiende visualmente la tabla~\ref{tab:transversal}---que reportaba solo las UEAs
    con tres o más licenciaturas---al incluir el conjunto completo de familias de contenido
    presentes en cinco o más planes.}
  \label{fig:matriz_transversal}
\end{figure}
...
Los veinte pares de la tabla~\ref{tab:inter} ilustran cuatro fenómenos distintos.
El bloque Ambiental--Química domina la parte superior con ocho pares, y cuenta con una docena adicional con $s \geq 0.70$ no incluidos en la tabla.
``Educación Ambiental y Cambio Climático'' aparece en tres instancias y está presente en al menos seis planes (tabla~\ref{tab:transversal}).
El par Computación--Física agrupa cuatro UEAs de formación amplia (Álgebra Lineal, Ética, Estadística, Liderazgo) con $s = 0.95$.
El par Civil--Mecánica en Ecuaciones Diferenciales Ordinarias expresa el núcleo matemático transversal documentado en la tabla~\ref{tab:transversal}.
...
La alta similitud intra-plan puede obedecer a causas legítimas---una pareja taller/teoría con el mismo nombre base constituye un par esperado---o a causas problemáticas: UEAs de distinto nivel con contenido no diferenciado, o duplicaciones documentales.

La figura~\ref{fig:histograma_intraplan} ofrece una vista comparativa de las distribuciones de similitud de los diez planes antes de examinar los casos individuales.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{Figuras/histograma_intraplan.png}
  \caption{Distribución de similitud intra-plan para las diez licenciaturas
    (pares con $s \geq 0.30$, estimación de densidad por núcleo gaussiano, KDE).
    \emph{Cómo leer la figura.}
    Cada panel corresponde a una licenciatura; el color sigue la paleta canónica del análisis.
    El \emph{eje $x$} es la puntuación de similitud $s$; el \emph{eje $y$} es la densidad
    relativa (el área bajo cada curva es 1 para facilitar la comparación entre planes
    con distinto número de pares).
    La \emph{línea vertical} marca la mediana de los pares de esa licenciatura.
    \emph{Comparación entre planes}:
    Civil presenta la distribución más ancha y con mediana más alta, consistente con sus
    61 pares y la heterogeneidad de sus áreas temáticas (estructuras, hidráulica, materiales);
    Electrónica muestra la distribución más compacta y desplazada a la izquierda, reflejo
    de sus 8 pares de similitud baja (máximo $s = 0.650$);
    Metalúrgica exhibe un pico pronunciado en $s \approx 0.75$--$0.80$, atribuible a las
    parejas taller/teoría del área de materiales, altamente homogéneas entre sí.
    La comparación entre paneles permite identificar qué planes concentran su traslape
    intra-plan en equivalencias de alta magnitud frente a aquéllos con distribuciones difusas
    de traslapes menores, orientando así la priorización de la revisión por plan.}
  \label{fig:histograma_intraplan}
\end{figure}
...
Los tres primeros pares de la tabla~\ref{tab:intra} son casos que requieren atención; los doce restantes son parejas taller/teoría confirmadas como legítimas por el LLM.
...
Dos UEAs consecutivas de laboratorio evaluadas como idénticas por el LLM.
...
Civil concentra 61 pares intra-plan a $s \geq 0.40$ (el máximo del corpus): el LLM detectó que las áreas de estructuras, hidráulica, geotecnia y materiales comparten objetivos convergentes redactados independientemente por departamentos distintos.
...
Las figuras~\ref{fig:intra_civil}--\ref{fig:intra_elecn} presentan los mapas de calor intra-plan de las diez licenciaturas, ordenados de mayor a menor densidad de traslape.

\paragraph{Cómo leer los mapas.}
Filas y columnas corresponden a las UEAs del plan, reordenadas por agrupamiento jerárquico de Ward, de modo que las UEAs de contenido similar quedan contiguas.
\emph{Diagonal}: siempre 1 (auto-similitud); una franja roja continua en la diagonal sin bloques fuera de ella indica un plan bien diferenciado.
\emph{Bloque compacto en la diagonal}: grupo de UEAs temáticamente afines (por ejemplo, un área disciplinar con varias materias relacionadas, o una secuencia taller/teoría).
\emph{Celda brillante aislada fuera de la diagonal}: par puntual de alta similitud---puede ser una pareja taller/teoría legítima o un duplicado.
\emph{Escala}: amarillo claro = baja similitud; rojo intenso = alta.
Los planes con más de 55 UEAs se presentan sin etiquetas de eje (ilegibles al ancho de página); la versión interactiva identifica cada UEA al pasar el cursor.
...
El vocabulario técnico especializado de Metalúrgica es heterogéneo entre versiones del mismo programa, lo que dificultaba la detección léxica; el LLM reconoció la equivalencia semántica subyacente.
Los 29 pares son en su totalidad parejas taller/teoría o áreas afines; no se identifican duplicados ni pares que requieran atención.
El par de mayor similitud es Lab.\@ de Tratamientos Térmicos/Tratamientos Térmicos de los Materiales ($s = 0.780$).
...
El LLM redujo los pares de 57 (TF-IDF) a 33, descartando 24 artefactos léxicos atribuibles al vocabulario genérico de desarrollo de software.
...
El par Programación Orientada a Objetos (POO) I vs.\ II es el par no-taller que presenta la diferenciación pedagógica más insuficiente.
...
La mayor parte del mapa es de color frío, lo que indica que los 108 contenidos están bien diferenciados para un plan de esta magnitud.
Los 25 pares son todos parejas taller/laboratorio esperadas: Lab.\@ de Mecanismos/Mecanismos ($s = 0.85$), Lab.\@ de Eficiencia Operativa/Medición del Trabajo ($s = 0.85$), Lab.\@ Estructura y Prop./Estructura y Prop.\@ de Mat.\@ ($s = 0.78$), Taller de Calefacción, Ventilación y Aire Acondicionado (CVAA)/CVAA ($s = 0.78$).
...
La celda más brillante fuera de la diagonal corresponde al par Aplicaciones Experimentales de Ingeniería Física I y II ($s = 0.950$, Caso~2).
...
El máximo intra-plan de Química ($s = 0.990$) es el más alto de todas las licenciaturas; corresponde al duplicado documental de Análisis de Ciclo de Vida (Caso~1).
...
Los 18 pares son en su mayoría parejas taller/teoría confirmadas: Lab.\@ de Metrología ($s = 0.85$), Lab.\@ de Conformado ($s = 0.85$), Lab.\@ de Eficiencia Operativa ($s = 0.85$) y Lab.\@ Estructura de Materiales ($s = 0.78$).
La celda más brillante fuera de las parejas taller/teoría corresponde al duplicado de Herramientas Digitales en Ingeniería (caso adicional).
...
El LLM redujo los pares de 27 (TF-IDF) a 14, descartando pares atribuibles al vocabulario electromagnético compartido por Circuitos, Teoría Electromagnética y Cálculo Vectorial sin equivalencia real de contenidos.
...
Traslape intra-plan bajo; los 7 pares corresponden a UEAs del área de gestión ambiental con objetivos semánticamente afines y redacciones heterogéneas.
Contrasta con el traslape inter-plan intenso con Química (14 UEAs a $s \geq 0.70$): los contenidos de Ambiental están bien diferenciados internamente, aunque se solapan significativamente con los de Química.
...
Electrónica presenta la menor densidad de traslape intra-plan del corpus (8 pares, máximo $s = 0.650$).
Sus contenidos están bien diferenciados internamente.
Sin embargo, su traslape inter-plan es intenso: 89 pares con otras licenciaturas, incluidos pares con $s = 1.00$.
...
El bloque más compacto e intenso del mapa, con barras laterales de dos colores (Ambiental y Química), confirma el traslape bilateral documentado en la tabla~\ref{tab:matriz_interplan}.
...
La diversidad de colores en la barra lateral evidencia el carácter inter-plan del traslape dentro de esta comunidad.
...
Corresponde al núcleo matemático transversal documentado en la tabla~\ref{tab:transversal}.
...
Los grupos visibles en la figura~\ref{fig:tsne} son coherentes con las comunidades identificadas en el mapa de calor agrupado: un grupo denso en la zona de Ambiental/Química, un bloque de UEAs matemáticas con puntos de múltiples licenciaturas superpuestos, y una región de Electrónica/Eléctrica/Física.
...
Las 38 UEAs equivalentes representan el 49\,\% de las UEAs no-TG de Industrial (78~UEAs) y el 35\,\% de las de Mecánica (108~UEAs).
\end{hallazgo}
...
\end{itemize}
La distribución comparativa de ambas puntuaciones para los 1\,505 pares, con la región de
detección semántica diferenciada, se muestra en la figura~\ref{fig:scatter_llm}.
\end{hallazgo}
...
De los hallazgos de la sección~\ref{sec:hallazgos} se derivan cinco líneas de acción para las coordinaciones de licenciatura y las instancias de revisión curricular de la División.
...
\textbf{2. Documentar el alcance de ``Educación Ambiental y Cambio Climático'' (H7).}
La UEA presenta contenido prácticamente idéntico en seis licenciaturas ($s \geq 0.95$); el hallazgo se documenta para conocimiento de las instancias de revisión curricular, que determinarán el tratamiento institucional correspondiente.
...
En ambos casos el análisis documenta el traslape; la determinación de su origen---decisión curricular explícita o resultado no intencional---corresponde a las instancias académicas competentes.

\textbf{5. Diferenciación de contenidos en Computación (H12).}
Los 33 pares intra-plan confirmados incluyen casos de diferenciación pedagógica insuficiente: POO I vs.\  II y los grupos de redes y bases de datos.
Se recomienda una revisión específica del plan de Computación para establecer diferenciación explícita entre UEAs consecutivas y entre UEAs de áreas temáticas próximas.
