...
La segunda es el \textit{recuento de pares equivalentes} entre licenciaturas a dos umbrales operacionales: $s \geq 0.30$ para pares inter-plan y $s \geq 0.40$ para pares intra-plan---umbral más estricto en el segundo caso, dado que la afinidad disciplinar entre UEAs del mismo plan es estructuralmente mayor que entre licenciaturas distintas.
La tercera es la \textit{similitud media-máxima} $\bar{s}(A,B)$ entre planes completos: para cada UEA del plan $A$ se identifica la UEA más similar que existe en $B$ y se promedia esa similitud máxima sobre todo $A$; el indicador es asimétrico por diseño---$\bar{s}(A,B)$ puede diferir de $\bar{s}(B,A)$ cuando los planes difieren en tamaño o diversidad---y cuantifica el grado de cobertura que $B$ ofrece para los contenidos de $A$.

Los hallazgos revelan un traslape más extenso y estructuralmente más complejo que el detectable por análisis léxico.
El par Industrial--Mecánica concentra la mayor densidad del corpus: 38~UEAs equivalentes a $s \geq 0.70$, seis veces más que el segundo par de mayor traslape, cubriendo manufactura, materiales, termodinámica, métodos numéricos y UEAs de formación amplia; la magnitud sugiere que ambos planes comparten implícitamente más de un tercio de sus contenidos fuera del Tronco General.
...
El bloque Ambiental--Química aporta la segunda densidad más alta (14~UEAs a $s \geq 0.70$), con contenidos de ingeniería ambiental compartidos que superan lo atribuible a vocabulario disciplinar común.
Cuatro familias de contenido cruzan entre cinco y nueve planes con alta similitud sin coordinación formal entre licenciaturas: Ecuaciones Diferenciales (9 planes, $s = 1.00$), Liderazgo y Emprendimiento (9 planes, detectable solo por el LLM porque TF-IDF le asignaba $s \approx 0.21$--$0.25$), Educación Ambiental y Cambio Climático (6~o más planes, $s \geq 0.95$), y un núcleo matemático de cuatro UEAs presentes en 5~planes cada una con $s \geq 0.98$.
...
El enriquecimiento semántico con LLM fue determinante: incrementó los pares inter-plan detectados en 154\,\% y los intra-plan en 75\,\%, descubriendo 455~pares con equivalencia semántica real que el análisis léxico habría descartado.

Todos los resultados de este reporte---tablas, mapas de calor, grafo, proyección t-SNE (\textit{t-distributed Stochastic Neighbor Embedding})---provienen de la matriz híbrida.
...
El par Programación Orientada a Objetos (POO) I vs.\ II permanece como el par no-taller más preocupante.
